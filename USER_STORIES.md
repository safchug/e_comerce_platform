...
- [x] **US-024**: As a shopper, I want to cancel an order before it ships, so that I'm not charged for mistakes.
  - AC: Cancellation reverses stock reservation; not allowed after "Shipped".
  - Learn: compensating actions, saga-lite thinking.
  - Status: the FSM (`Order.ALLOWED_TRANSITIONS`) and the stock-reversal compensating action already existed from US-023, but the only way to reach `CANCELLED` was the admin-only `PATCH /orders/:id/status` - a shopper had no way to cancel their own order. Added `CancelOrderUseCase` (`src/modules/order/application/use-cases/cancel-order.use-case.ts`) and a shopper-facing `POST /orders/:id/cancel` (`OrderController`, behind plain `JwtAuthGuard`, no admin role required), which reuses `Order.withStatus`/`OrderRepository.updateStatus` for the transition and stock reversal - no new saga/compensation logic needed, US-023 already built the compensating action, this story was really "expose it safely to its actual owner." The interesting design call was ownership: `findById` isn't user-scoped, so a shopper could otherwise pass another user's order id. Rather than a 403 (which confirms the order exists but isn't yours), a mismatched `userId` is reported the same way as a missing order - `OrderNotFoundError` -> 404 - to avoid leaking other users' order ids via response-code oracle. Covered end-to-end: cancel from PENDING and PAID (stock restored), rejected with 409 once SHIPPED (reusing the existing transition-table error), rejected with 404 for someone else's order id or a missing one, and 401 with no token.
